  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center gap-3 mt-24">
        ...
        <p className="text-slate-400 text-sm">Building dashboard interface...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-8 py-2">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">Welcome back, <span className="text-emerald-400 font-semibold">{user.name}</span>. Here is your active daily regimen.</p>
        </div>
        ...
        {activePlan && (
          <button 
            ...
            className="btn-secondary py-2.5 px-4 text-xs font-semibold flex items-center gap-2 hover:border-emerald-500 hover:text-emerald-400"
          >
      ...
      {pendingPlan && (
        <div className="p-4 rounded-xl border border-amber-500/25 bg-amber-500/10 text-amber-300 text-sm flex gap-3 items-start">
          <Clock className="w-5 h-5 text-amber-500 shrink-0 mt-0.5 animate-pulse" />
          <div>
            <h4 className="font-bold text-amber-400 mb-0.5">Clinical Review In Progress</h4>
      ...
      {!activePlan && !pendingPlan ? (
        ...
        <div className="glass-panel p-12 text-center flex flex-col items-center gap-6 max-w-2xl mx-auto mt-6">
          <Apple className="w-16 h-16 text-slate-600 animate-pulse" />
          <div>
                ...
                {activePlan.meals?.map((meal, idx) => (
                  <div key={idx} className="p-4 rounded-xl border border-slate-900 bg-slate-900/10 flex flex-col gap-2">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-900/60">
                      <span className="font-bold text-slate-200 font-heading">{meal.type}</span>
                      <span className="text-xs font-semibold text-emerald-400 px-2 py-0.5 rounded bg-emerald-500/5 border border-emerald-500/20">{meal.kcal} kcal</span>
                    </div>
                    <ul className="list-disc list-inside text-xs text-slate-400 flex flex-col gap-1.5 mt-1">
            ...
            <div className="glass-panel p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-4">
                {activePlan.workout?.map((work, idx) => (
                  <div key={idx} className="p-4 rounded-xl border border-slate-900 bg-slate-900/10 flex flex-col md:flex-row justify-between gap-3">
                    <div className="min-w-[120px]">
                      <span className="text-xs uppercase font-bold text-slate-500 tracking-wider">{work.day}</span>
                      <h4 className="font-bold text-slate-200 font-heading text-sm mt-0.5">{work.focus}</h4>
                    </div>
                    <div className="flex-1">
                      <ul className="text-xs text-slate-400 flex flex-wrap gap-2">
                        {work.exercises.map((ex, exIdx) => (
                          <li key={exIdx} className="bg-slate-900/50 px-2.5 py-1.5 rounded-lg border border-slate-900/40 text-slate-300">
                            {ex}
                          </li>
                        ))}
                      </ul>
